...
const DECORATIVE_ICONS = [
  { icon: Server, x: '8%', y: '12%' },
  { icon: Database, x: '88%', y: '15%' },
  { icon: ShieldAlert, x: '12%', y: '80%' },
  { icon: Cpu, x: '82%', y: '82%' },
  { icon: Network, x: '15%', y: '45%' },
  { icon: Boxes, x: '78%', y: '35%' },
  { icon: Globe, x: '5%', y: '30%' },
  { icon: Terminal, x: '92%', y: '50%' },
]

const SYSTEM_CHIPS = [
  { text: '0x404_VOID', x: '18%', y: '25%' },
  { text: 'COORD_LOST', x: '72%', y: '15%' },
  { text: 'SYS_INTEGRITY: OK', x: '25%', y: '70%' },
  { text: 'V5_CORE_ACTIVE', x: '78%', y: '60%' },
]
  ...
  return (
    <AppContainer>
      <Container className="relative min-h-[90vh] flex items-center justify-center overflow-hidden py-10">
        ...
        <div className="absolute inset-0 pointer-events-none hidden lg:block overflow-hidden">
          {DECORATIVE_ICONS.map((item, i) => (
            <div
              ...
              style={{ left: item.x, top: item.y, animationDelay: `${i * 0.4}s` }}
            >
          ...
          {SYSTEM_CHIPS.map((chip, i) => (
            <div
              key={`bg-chip-${i}`}
              className="absolute animate-pulse"
              style={{ left: chip.x, top: chip.y, animationDelay: `${i * 0.6}s` }}
            >
        ...
        <div className="animate-in fade-in zoom-in-95 duration-700 relative z-10 w-full">
