...
- **`symfony`** (`php:8.4-fpm-alpine`):
  ...
  - Inicia via `devops/bootstrap.sh`: ajusta permissões de cache e logs, e gera chaves JWT se necessário.
...
2. **Bootstrap**: O script `devops/bootstrap.sh` é disparado, garantindo que as chaves RS256 para o JWT Auth estejam presentes no diretório `config/jwt`.
